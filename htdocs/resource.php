...
if (isset($_GET["resourceaddaccessresult"]) and $_GET["resourceaddaccessresult"]) {
    $resourceaddaccessresult = $_GET["resourceaddaccessresult"];
}
...
$smarty->assign('use_resetpassword', $use_resetpassword);
    ...
    if ($ldap) {
        ...
        # Search attributes
        $attributes = array();
        $search_items = array_merge( $resource_items, $display_password_items);
        foreach( $search_items as $item ) {
            $attributes[] = $attributes_map[$item]['attribute'];
        }
        $attributes[] = $attributes_map[$resource_title]['attribute'];
#        $attributes[] = "pwdPolicySubentry";
        ...
        foreach ($entry[0] as $attr => $values) {
            ...
            $entry[0][$attr] = $values;
	    if ( $attr == "racfaccesscontrol" ) {
	        foreach ( $entry[0][$attr] as &$value ) {
$texts = preg_split('/ID\((.*)\) ACCESS\((.*)\) COUNT\((.*)\)/i', $value, NULL, PREG_SPLIT_DELIM_CAPTURE);
		    $value = array("");
                    $value['id'] = $texts[1];
		    $value['acc'] = $texts[2];
		    $value['count'] = $texts[3];
...
$smarty->assign("card_title", $resource_title);
$smarty->assign("card_items", $resource_items);
...
$smarty->assign("resourceaddaccessresult", $resourceaddaccessresult);
?>
